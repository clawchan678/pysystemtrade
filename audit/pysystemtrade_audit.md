...
Status: **P0 Phases 1–6 complete** (Phase 4 and Phase 5 gates approved by the operator). Next: Phase 7.
...
18. **Evidence gaps (after Phase 6).** Still classified-only: pooled forecast correlation cleaning path, `pandl_SR_cost.py`, risk-series internals, data/roll construction (D01), compounding capital (P07), and dynamic optimisation (P09, PARTIALLY AUDITED). The optimiser internals were read in Phase 6; R03/R05/R06 stay INFERRED per §12 (U5).
...
NOT YET AUDITED (Phase 7). Pre-flags: forecast-scalar backfill (`forecast_scalar.py:48`); vol `backfill` default False (`vol.py:128`); full-sample turnover (`strategy_functions.py:31`); full-history instrument-length filter (`basesystem.py:373-390`, default off); **added in Phase 5:** cash-cost vol deflator divides by the final sample vol (`strategy_functions.py:164-172`, active by default via `vol_normalise_currency_costs: True`); SR cost per trade uses the last year of price and vol (`account_costs.py:319-340`); correlation `cleaning` path uses `self.data` (UNVERIFIED). All are unclassified pending Phase 14.
